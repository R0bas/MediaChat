const {SlashCommandBuilder, Attachment, AttachmentBuilder} = require('discord.js');
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('sendyt')
        .setDescription('Envoyer une video youtube')
        .addStringOption((option) => option
        .setName('url').setDescription('Lien de la video Youtube')
            .setRequired(true))
        .addStringOption(option => option
            .setName('positionx')
    ...
    async execute(interaction) {
        ...
        let image = undefined
        await get_mp4_url(url);

            
            
            const my_video = fs.readFileSync(`./static/video.mp4`);
            const msg = interaction.channel.send({content: "Je veux mourir :)", files: [new AttachmentBuilder(my_video, {name: "mourir.mp4"})], ephemeral: true});
            const my_message = await msg;
            const discord_url =Array.from(my_message.attachments)[0][1].url
        ...
        try {
            return interaction.reply("<@" + interaction.user + "> à envoyé le fichier " + discord_url + " à " + (user != null ? user.username : "tout le monde") + (text != null ? " avec le texte : ```" + text + " ```" : ""));
